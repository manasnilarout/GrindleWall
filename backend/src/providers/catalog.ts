 ...
 *
 * Hand-transcribed from the docs on 2026-09-05 and still NOT probed as a table.
 * Exactly two of these thirty — `Kore` and `Zephyr` — have been accepted live,
 * which is what establishes the TitleCase spelling and nothing else. Google
 * publishes no voice list endpoint, so unlike the Murf and Cartesia tables
 * (generated by a script from each vendor's own list) the other twenty-eight
 * are only as good as the doc page they came from.
 *
 ...
 * Read off the docs on 2026-09-05 and still not probed as a table. Live round
 * trips have since run through this provider, but only in `en-US` — one locale
 * out of the list below, which says nothing about the other eighty-two.
...
export const CATALOG: ProviderEntry[] = [
  ...
  },
  {
    ...
    //
    // Verified live 2026-09-05: both model ids exist per GET /v1/models, and
    // `gpt-realtime-2.1` and `-mini` have each completed a turn — see
    // `npm run realtime:probe`. The nested GA session shape below is therefore
    // confirmed accepted, not merely documented. Voice and language coverage
    // are still doc-derived: only `marin` has actually been heard. The
    ...
    models: [
      { id: 'gpt-realtime-2.1', name: 'gpt-realtime-2.1', note: 'Current flagship speech-to-speech' },
      {
        ...
        note: 'Distilled — audio at $10/$20 per 1M vs the flagship\u2019s $32/$64, and claims better alphanumeric recognition',
      },
    ],
